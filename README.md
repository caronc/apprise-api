...
| `APPRISE_ALLOW_CONFIG_LIST` | Allow listing stored configuration keys. This is only valid when `STATEFUL_MODE` is set to `simple`. This defaults to `no` and can be set to `yes`.
